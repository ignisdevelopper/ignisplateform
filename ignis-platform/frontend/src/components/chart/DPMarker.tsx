...
export type DPMarkerProps = {
  dp: DPResult;

  /** If provided => marker will be positioned absolutely (useful for chart overlay) */
  y?: number;
  x?: number;
  ...
  /** interactions */
  onSelect?: (dp: DPResult) => void;
  onFocusPrice?: (price: number, dp: DPResult) => void;
...
export default function DPMarker({
  dp,
  y,
  x,
  side = 'right',
  ...
  const color = useMemo(() => dpTypeColor(dp.dp_type), [dp.dp_type]);
  const pillCls = useMemo(() => dpTypePill(dp.dp_type), [dp.dp_type]);

  const dist = useMemo(() => {
    if (currentPrice === undefined || currentPrice === null) return undefined;
    if (!Number.isFinite(currentPrice) || !Number.isFinite(dp.price)) return undefined;
    return Math.abs(currentPrice - dp.price);
  }, [currentPrice, dp.price]);

  const metaPreview = useMemo(() => {
    if (!showMetaPreview) return null;
    const m = dp.meta ?? {};
    const direction = pickString(m, ['direction', 'bias', 'side']);
    const reason = pickString(m, ['reason', 'notes', 'message', 'comment']);
    const strength = pickNumber(m, ['confidence', 'strength']);
    const levelKind = pickString(m, ['kind', 'level_kind']);
    return { direction, reason, strength, levelKind };
  }, [dp.meta, showMetaPreview]);
  ...
  const title = `${dp.dp_type} · ${fmt(dp.price, 6)} · ${fmt(dp.score, 0)}%`;
  ...
  return (
    <motion.div
      ...
      title={title}
    >
      {/* Connector (small line to the chart scale) */}
      {absolute && (
        <div
          className={cn(
      ...
      <button
        ...
        style={{
          boxShadow: selected ? `0 0 0 1px ${hexToRgba(color, 0.25)} inset` : undefined,
        }}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className={cn('mt-2 flex flex-wrap items-center gap-2', compact ? 'text-xs' : 'text-sm')}>
              <div className="font-semibold text-white/90">
                {fmt(dp.price, compact ? 4 : 6)}
              </div>

              <span className="text-white/30">·</span>

              <div className="text-white/75">
                score <span className="text-white/90 font-semibold">{fmt(dp.score, 0)}%</span>
              </div>
            ...
            {!compact && metaPreview && (metaPreview.direction || metaPreview.levelKind) && (
              <div className="mt-1 text-[11px] text-white/55 truncate">
                {metaPreview.direction ? `dir ${String(metaPreview.direction).toUpperCase()}` : 'dir —'}
                {metaPreview.levelKind ? <span className="text-white/35"> · {metaPreview.levelKind}</span> : null}
                {metaPreview.strength !== undefined ? (
                  <span className="text-white/35"> · conf {fmtPct(metaPreview.strength, 0)}</span>
                ) : null}
              </div>
            )}
          </div>
          ...
          <div className="min-w-[96px] text-right">
            ...
            <div className="mt-1">
              <ScoreBar value={dp.score} color={color} />
            </div>
          </div>
        </div>
            ...
            >
              {metaPreview?.reason && (
                <div className="rounded-xl border border-white/10 bg-black/25 p-3">
                  <div className="text-[11px] uppercase tracking-wide text-white/45">Reason</div>
              ...
              {showActions && (
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => copyToClipboard(String(dp.price))}
                  ...
                  <button
                    type="button"
                    onClick={() => onFocusPrice?.(dp.price, dp)}
                    ...
                    title={!onFocusPrice ? 'onFocusPrice non fourni' : 'Focus chart on this DP'}
                  >
                    Focus chart
                  </button>
                </div>
  ...
  return (
    <div className="h-2 rounded-full border border-white/10 bg-white/5 overflow-hidden">
      <div
        ...
        style={{
          width: `${v}%`,
          background: `linear-gradient(90deg, ${hexToRgba(color, 0.75)} 0%, ${hexToRgba(
            color,
            0.20
          )} 100%)`,
        }}
...
function dpTypeColor(type: DPType) {
  switch (type) {
    case 'SDP':
      return '#1D9E75';
    case 'SB_LEVEL':
...
function dpTypePill(type: DPType) {
  switch (type) {
    case 'SDP':
